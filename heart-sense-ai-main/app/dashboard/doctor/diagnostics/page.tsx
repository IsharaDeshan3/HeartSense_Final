...
export default function DiagnosticsPage() {
  ...
  return (
    <div
      className={`min-h-screen bg-background text-foreground p-10 space-y-12 relative overflow-hidden clinical-access ${isNightMode ? "night-mode" : ""}`}
    >
      {/* Background Ambience */}
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 max-w-7xl mx-auto">
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            ...
            <span className="text-primary">ECG Diagnostics</span>
          </div>
          <h1 className="text-4xl font-extrabold tracking-tight italic">
            ECG <span className="text-primary">Analysis</span>
          </h1>
          <p className="text-muted-foreground text-sm max-w-lg">
            Leverage high-performance neural synthesis for noise-robust cardiac
        ...
        <div className="flex items-center gap-4 border border-border px-6 py-4 rounded-[2rem] bg-card shadow-sm">
          ...
          <div className="text-right">
            <p className="text-xs font-bold text-accent tracking-widest uppercase italic">
              Neural Sync Active
            </p>
            <p className="text-[10px] text-muted-foreground uppercase opacity-50">
              HeartSense-v2.0 // Gemini-Flash
            </p>
          </div>
