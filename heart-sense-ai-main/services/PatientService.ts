...
export const PatientService = {
  ...
  async fetchPatientHistory(patientId?: string): Promise<any[]> {
    const auth = getAuth();
    if (!auth) {
      console.log("[PatientService] fetchPatientHistory: No auth, returning empty array");
      return [];
    }
    ...
    try {
      ...
      console.log("[PatientService] fetchPatientHistory response status:", res.status);

      if (!res.ok) {
        const errorText = await res.text().catch(() => "");
        console.error("[PatientService] fetchPatientHistory failed:", res.status, errorText);
        return [];
      }
      ...
      console.error("[PatientService] fetchPatientHistory network error:", error);
      return [];
    }
  },
  ...
  async savePatientHistory(payload: {
    ...
    const auth = getAuth();
    if (!auth) {
      console.error("[PatientService] savePatientHistory: No auth - cannot save");
      return null;
    }

    const targetUserId = payload.patientId ?? payload.userId ?? payload.user_id ?? auth.userId;
  ...
  async sendDiabeticData(extractedJsonGroup1: any, patientId?: string) {
    ...
    if (!res.ok) {
      const errorText = await res.text();
      console.error("[PatientService] sendDiabeticData failed:", res.status, errorText);
      return null;
    }
  ...
  async sendHeartData(extractedJsonGroup2: any, patientId?: string) {
    ...
    if (!res.ok) {
      const errorText = await res.text();
      console.error("[PatientService] sendHeartData failed:", res.status, errorText);
      return null;
    }
