...
def get_store() -> LocalFeedbackStore:
    # Admin review uses the same local feedback store as the submission route.
    global _store
    if _store is None:
...
def _require_admin(x_admin_token: Optional[str]) -> None:
    # Local admin access is gated by a simple token check instead of auth.
...
async def list_pending(limit: int = 50, x_admin_token: Optional[str] = Header(default=None)):
    # Lists queued feedback items so an operator can review what local runs
    # produced before approving or rejecting them.
...
async def approve(req: ApproveRequest, x_admin_token: Optional[str] = Header(default=None)):
    # Approval updates the same local store; it does not touch Supabase.
    _require_admin(x_admin_token)
